...
export const localStorage = (id, count) => {
  ...
  if (actualProduct.type == "ring") {
    ...
    if (document.querySelector(".prod__select").value == "Select") {
      ...
      taskStatus = true;
      actualProduct.clothingSize =
        document.querySelector(".prod__select").value;
    }
  } else if (actualProduct.type == "shoes") {
    if (document.querySelector(".prod__select").value == "Select") {
      document.querySelector(".prod__select").focus();
      taskStatus = false;
      alert("Select your shoes size");
    } else {
      taskStatus = true;
      actualProduct.shoesSize = document.querySelector(".prod__select").value;
    }
  }
  if (taskStatus == false) {
    return taskStatus;
  ...
  if (cart) {
    ...
    var isFindProduct;
    if (actualProduct.type == "ring") {
      isFindProduct = cart.some(
        (product) =>
          product.id === id && product.ringSize == actualProduct.ringSize
      );
    } else if (actualProduct.type == "clothing") {
      isFindProduct = cart.some(
        (product) =>
          product.id === id &&
          product.clothingSize == actualProduct.clothingSize
      );
    } else if (actualProduct.type == "shoes") {
      isFindProduct = cart.some(
        (product) =>
          product.id === id && product.shoesSize == actualProduct.shoesSize
      );
    } else {
    ...
    const updateCart = [
      ...cart,
      { ...actualProduct, quantity: count ? Number(count) : 1 },
    ];
    window.localStorage.setItem("cart", JSON.stringify(updateCart));
    return;
  }
